  ...
  return (
    <motion.div 
      ...
      className={cn(
        ...
        isFull ? "border-red-200 bg-red-50/30 dark:bg-red-900/10" : "border-border"
      )}
    >
      <div className="flex justify-between items-start mb-4">
        <div>
          <h3 className="font-serif font-semibold text-lg text-foreground">{zone.name}</h3>
          <p className="text-xs text-muted-foreground uppercase tracking-wider font-medium">
            Status: <span className={cn(
              "font-bold",
              isFull ? "text-destructive" : isNearFull ? "text-orange-500" : "text-primary"
            )}>
